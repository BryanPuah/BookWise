...
const NOTE_TYPES = {
  quote: {
    ...
    color: "#6B5B95",
    bg: "#F0EDF8",
    desc: "Direct words from the book",
  },
  insight: {
    label: "Insight",
    icon: "💡",
    color: "#2874A6",
    bg: "#EAF4FB",
    desc: "Your own interpretation or realisation",
  },
  question: {
    label: "Question",
    icon: "🔍",
    color: "#A04000",
    bg: "#FDF0E6",
    desc: "Something you want to explore further",
  },
  action: {
    label: "Action",
    icon: "✅",
    color: "#1E8449",
    bg: "#E9F7EF",
    desc: "Something you will apply or do",
  },
  summary: {
    label: "Summary",
    icon: "📌",
    color: "#76448A",
    bg: "#F5EEF8",
    desc: "Key takeaway from a chapter or section",
  },
  connection: {
    label: "Connection",
    icon: "🔗",
    color: "#17A589",
    bg: "#E8F8F5",
    desc: "This idea connects to another book or note",
  },
};
        ...
        <ScrollView style={m.scroll} keyboardShouldPersistTaps="handled">
          ...
          {book && (
            <View style={m.bookCtx}>
              ...
              <Text style={m.bookCtxTitle} numberOfLines={1}>
                {book.title}
              </Text>
            </View>
          )}
          ...
          >
            {Object.entries(NOTE_TYPES).map(([key, val]) => (
              ...
              >
                <Text
                  style={[m.typePillIcon, type === key && { color: "#fff" }]}
                >
                  {val.icon}
                </Text>
                <Text
                  style={[m.typePillTxt, type === key && { color: "#fff" }]}
                >
                  {val.label}
                </Text>
              </TouchableOpacity>
          ...
          <TextInput
            ...
            placeholder={
              type === "quote"
                ? "Type or paste the exact quote..."
                : type === "question"
                  ? "What do you want to explore further?"
                  : type === "action"
                    ? "What will you actually do with this?"
                    : type === "summary"
                      ? "Summarise this chapter in your own words..."
          ...
          <Text style={m.sectionLabel}>
            MY THINKING{" "}
            <Text style={m.optional}>(optional — but powerful)</Text>
          </Text>
          <TextInput
            ...
            placeholder={
              type === "quote"
                ? "Why does this quote matter to you? How does it connect to your life?"
                : type === "question"
                  ? "What do you already think the answer might be?"
                  : type === "action"
                    ? "Why this action? What outcome are you hoping for?"
                    : type === "summary"
                      ? "What surprised you? What will you remember in 6 months?"
      ...
      <View style={nc.body}>
        <View style={nc.header}>
          ...
          <View style={{ flexDirection: "row", gap: 8, alignItems: "center" }}>
            ...
            {note.starred ? (
              <Text style={{ fontSize: 14, color: "#D4A800" }}>★</Text>
            ) : null}
          </View>
        </View>

        {note.chapter ? (
          <Text style={nc.chapterTxt}>{note.chapter}</Text>
        ) : null}

        <Text
          style={[nc.mainTxt, note.type === "quote" && nc.quoteTxt]}
        ...
        {(note.text.length > 160 ||
          (note.thinking && note.thinking.length > 100)) && (
          <TouchableOpacity
            onPress={() => setExpanded((e) => !e)}
            activeOpacity={0.7}
          >
            <Text style={nc.expandBtn}>
        ...
        <View style={m.header}>
          ...
          >
            <Text
              style={[
                m.save,
                (!question.trim() || !answer.trim()) && m.saveDisabled,
              ]}
            >
              Save
            </Text>
      ...
      {[1, 2, 3, 4, 5].map((s) => (
        ...
        >
          <Text
            style={{
              fontSize: 16,
              color: s <= rating ? C.amber : "rgba(255,255,255,0.3)",
            }}
          >
            {s <= rating ? "★" : "☆"}
          </Text>
          ...
          <LinearGradient colors={[C.heroTop, C.heroBot]} style={s.hero}>
            ...
            <View style={s.heroBody}>
              ...
              <View style={s.chips}>
                ...
                {book.pageCount > 0 && (
                  <View style={s.chip}>
                    <Text style={s.chipTxt}>{book.pageCount} pages</Text>
                  </View>
                )}
              </View>
            ...
            {(book.status === "reading" || book.status === "finished") && (
              <View style={s.progressSection}>
                ...
                <View style={s.progressInputRow}>
                  ...
                  <TouchableOpacity
                    style={[s.updateBtn, { alignSelf: "flex-end" }]}
          ...
          <View style={s.body}>
              ...
              <>
                {notes.length === 0 && (
                  <Text style={s.emptyTxt}>
                    No notes yet. Tap "Add a note" in the book header to capture
                    your first thought.
                  </Text>
                )}
            ...
            {tab === "cards" && (
              <>
                {cards.length === 0 && (
                  <Text style={s.emptyTxt}>
                    No flashcards yet. Make them from your notes!
                  </Text>
                )}
                {cards.map((c) => (
